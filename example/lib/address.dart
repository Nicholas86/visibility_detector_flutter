...
typedef NextLevelDataFetcher = FutureOr<List<RegionData>?> Function(RegionData data, int level, int index);
...
class AddressWidget extends StatefulWidget {
  const AddressWidget({
    ...
    this.tabHeight = 40.0,
  }) : assert(provinceHasAllData || fetchNextLevelData != null, '当provinceHasAllData为false时fetchNextLevelData不能为null');
...
class _AddressWidgetState extends State<AddressWidget> with SingleTickerProviderStateMixin {
  ...
  // 当前选中的地区集合
  List<RegionData> get currentSelectedRegion => _selectedRegions.whereType<RegionData>().toList();
  ...
  double get tabOffset {
    final tabLeft = (widget.tabPadding?.left ?? 0) + (widget.tabItemMargin?.left ?? 0) + (widget.tabItemPadding?.left ?? 0);
    final tabRight = (widget.tabPadding?.right ?? 0) + (widget.tabItemMargin?.right ?? 0) + (widget.tabItemPadding?.right ?? 0);
  ...
  TextStyle _styleForSelected(bool selected) => selected ? widget.selectedTextStyle : widget.textStyle;
    ...
    return AfterLayout(
      ...
      callback: (val) {
        ...
        if (maxScrollExtent > 0) {
          _scrollController.animateTo(maxScrollExtent, duration: widget.duration, curve: Curves.ease);
        }
      },
    );
  }
          ...
          return InkWell(
            ...
            child: widget.itemBuilder?.call(item, selected, _currentTab) ??
                Container(
                  ...
                  child: Row(
                    children: [
                      if (widget.showLoadingWidget && selected && isLoading) widget.loadingWidget ?? defaultLoadingWidget,
                      if (widget.showCheckWidget && selected && !isLoading) widget.checkWidget ?? defaultCheckedWidget,
                      Text(
                        item.name ?? '',
